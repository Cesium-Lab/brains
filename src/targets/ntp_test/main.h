...
char timeServer[] = "132.163.96.1"; // time.nist.gov NTP server

in_addr_t time_server_addr = inet_addr("132.163.96.1");
...
int main() {
    ...
    // Set sending port
    int SEND_PORT = 8080;
    serveraddr.sin_port = htons(SEND_PORT); // Turns little endian int to big endian for networking
    ...
    socklen_t serversize = sizeof(struct sockaddr_in); // Temp buffer for recvfrom
