...
// Turns two `uint8_t` into a `uint16_t`. Must make sure there are 2 bytes in the buffer
...
// Turns two `uint8_t` into a `float`. Must make sure there are 2 bytes in the buffer
float bytes_to_float(uint8_t* buf) {
    return static_cast<float>(bytes_to_u16(buf));
}

// Turns two `uint8_t` into a `float`.
float bytes_to_float(uint8_t upper, uint8_t lower) {
    return static_cast<float>(bytes_to_u16(upper, lower));
}